...
`git clone git@github.com:W4118/f25-hmwk1-UserName.git` (Replace
...
3.  **Using your floppy image to boot in QEMU.** You could reboot your
    ...
    Then you can use the floppy image to boot an x86 machine using QEMU:
        ...
        qemu-system-x86_64 -drive file=floppy.flp,format=raw -display curses

    The `-display curses` option tells QEMU to render the VGA text
    console directly in your terminal using ncurses instead of opening a
    graphical window, so this command works over SSH and does not
    require a graphical user interface on your VM. Do **not** use the
    `-nographic` option: it detaches the VGA console entirely and you
    will see nothing. Other than the cross-compilation, the other
    instructions are the same for both x86 hardware users and Arm
    hardware users. You should be able to boot using QEMU with the Hello
    World OS you built and see \"hello, world\" printed on your console.

    A few practical notes about `-display curses`:

    - Your terminal window must be at least 80 columns by 25 rows, the
      size of the VGA text console. If it is smaller, QEMU only shows
      the portion of the console that fits, and your centered output may
      be clipped or appear off-center. Run `stty size` to check; it
      prints rows followed by columns.
    - To exit QEMU, press `Ctrl-A` and then `X`. Terminating QEMU this
      way restores your terminal properly; killing it from another
      window may leave your terminal in a strange state, which you can
      fix with `reset`.
    - Colors are rendered by your terminal emulator rather than by a VGA
      chip, so the exact shades may differ from the reference
      screenshots in this assignment.
      What must be true is the *relationship*: the characters you print
      must be dark on a light block, and the separator spaces before the
      `ip` and `sp` values must be on a black background. Your work is
      graded from the actual bytes in the VGA framebuffer, not from a
      screenshot, so a terminal whose palette looks slightly different
      will not cost you points.
    - If QEMU exits immediately with an error such as
      `Display 'curses' is not available`, your QEMU was built without
      curses support. Run `qemu-system-x86_64 -display help` to list the
      display backends your build supports, and contact the
      instructional staff.
