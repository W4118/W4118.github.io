...
Take a look at the [`remap_pfn_range`](https://linux-kernel-labs.github.io/refs/pull/222/merge/labs/memory_mapping.html) function to implement this remapping. The function takes several arguments, including the `vma` representing the user-space region to use, the starting virtual address to use, and the `pfn` representing the starting page frame number of the physical memory region to map. It is well documented for use in kernel drivers, but it can also be used elsewhere. Look at how it is implemented and try to understand in broad terms how it works (this will be useful for the next parts of the assignment). Also look at usages across the kernel, for example [here](https://elixir.bootlin.com/linux/v6.14/source/include/linux/mm.h#L3580) (called [here](https://elixir.bootlin.com/linux/v6.14/source/mm/memory.c#L2878)).  
...
- Find the VMA (`vma_lookup`/`find_vma`) containing the memory to be used in `remap_pfn_range`, and tailor it for remapping (`vma_modify_flags`) so that the VMA contains exactly the range of addresses to be remapped. See notes below on how to use `vma_modify_flags`. `vm_flags_clear` could also be useful to clear flags.
- Check that no PTEs have been allocated in the VMA so that our remapping does not corrupt any existing user mappings. You may find it helpful to review kernel functions such as `__handle_mm_fault`, `__do_page_fault`, `show_pte(arm64)` for walking page tables.  If any PTEs for the VMA have been allocated, you should return an error as `remap_pfn_range` will not work in that case.
- Modify the VMA flags so that it cannot be written from userspace. You may also want to restore the original VMA flags later so the userspace process can use the memory normally after tracking ends.
- `virt_to_phys` could be useful for converting virtual to physical addresses.
...
*   Only one process can have mapped memory at a time. If a process calls the system call when another process has already `open`ed some shared memory, it should receive `-EBUSY`.
*   Only a superuser should be able to make the system call, otherwise, it should give `-EPERM`.
*   `dest` should be page aligned. Otherwise, return `-EINVAL`.
...
*   `remap_pfn_range` is done at page granularity, so the actual required memory size needs to be rounded up to the next page boundary.
*   The `vma_modify` function takes in attributes (flags, start address, end address, etc.) about what some section of a `vma` will look like _after_ you modify it, and either merges it with its neighbors (if attributes match) or splits it based on start and end to create an isolated vma with the new attributes. In this case, focus on how to parameterize `vma_modify` to guarantee that the memory you will remap is isolated in exactly one `vma`. The `vma_modify_flags` function is particularly useful for as it calls `vma_modify` and sets specific attributes for the `vma` (look at `VM_SPECIAL`).
...
<!-- PART 1: Playing with virtual memory
------

Before building the shadow page table, implement a system call to walk a given process’s page table, and translate a virtual address to its physical frame number.

```c
/*
 * Syscall No. 467
 * Retrieve the physical frame number (PFN) for a given virtual address
 */
long shadowpt_get_pfn(pid_t proc, unsigned long va);
```

This system call should take in a pid and a virtual address. If the caller has superuser permissions, and the virtual address is mapped to a physical address, it should return the physical address.  Otherwise, it should return relevant errno values in failure cases.
the provided pfn is currently mapped by the given process, then the system call should copy the contents of that page into the userspace buffer. 

With `shadowpt_get_pfn`, you should be able to obtain the virtual-to-physical mapping for some process' virtual address. To help test this functionality, you should also implement the following system call:
 ...
 */
long shadowpt_page_contents(pid_t proc, unsigned long pfn, void __user *buffer);
 ```

This system call should take in a pid, physical address and a userspace buffer. If the caller has superuser permissions, and the provided pfn is currently mapped by the given process, then the system call should copy the contents of that page into the userspace buffer. Otherwise, it should return relevant errno values in failure cases.

**Notes**

* To implement `shadowpt_get_pfn`, walk the current process’s page tables using helpers such as `pgd_offset()`, `p4d_offset()`, `pud_offset()`, `pmd_offset()`, and `pte_offset_map()`.
* Use `pte_pfn()` to get the pfn associated with a pte
* Hold the proper locks (`mmap_read_lock`).
* You’ll reuse this page-table walking logic in later parts (e.g., `update_shadow_pte`).

**Tasks**

1.  Implement `shadowpt_get_pfn` to get the virtual to physical address mapping.
2.  Implement `shadowpt_page_contents` to get the contents from a physical page. -->
...
*   Call `shadowpt_open` with the target PID and the configured `user_shadow_pt` struct.
*   Upon success, enter a loop that prints the contents of the shadow page table once per second.
*   Implement a signal handler for `SIGINT` (ctrl+c) that ensures `shadowpt_close` is called before the program exits, so kernel resources are cleaned up properly.
...
*   When reading from components of the memory management system, remember that you are not the only one using them. Make sure that you grab the appropriate locks and increment reference counts as necessary while accessing shared kernel data structures.
...
* **After `mmap`**: `SPTE_VADDR_ALLOCATED` is set (and `SPTE_VADDR_ANONYMOUS` if anonymous); `SPTE_PTE_MAPPED` is unset and `pfn` is 0.
* **After first-touch write**: `SPTE_PTE_MAPPED` becomes set and `pfn` is populated.
...
**Notes:**
*   Make sure to unset `SPTE_VADDR_ALLOCATED` in the `state_flags` field when the address is deallocated (i.e. after an `free` call). 
...
**Notes:**
*   Make sure to unset `SPTE_PTE_MAPPED` when the mapping for a virtual page to a physical page is removed.
*   There is a huge number of places in the kernel where PTEs are invalidated, and placing an update after each one would be extremely time consuming. Luckily, we’re not the only ones interested in page tables, and as a result the kernel provides the [MMU notifier](https://elixir.bootlin.com/linux/v7.0/source/include/linux/mmu_notifier.h), which allows us to register our own callback functions. These callback functions will be called whenever a particular range of PTEs is going to be invalidated. 
...
* `mmap`/`munmap` allocations
* `malloc`/`free` (heap growth via `brk`)
* Stack expansion (`VM_GROWSDOWN`/`VM_GROWSUP`)
* `execve` replacement
* Process `exit`
* MMU notifier invalidations
...
------
Write answers to the following questions in `user/part5.txt`, following the provided template **exactly**. Make sure to include any references you use in your references.txt file, and that you are referencing the correct kernel version in bootlin ([v7.0](https://elixir.bootlin.com/linux/v7.0/source)). For reference, the URLs you answer with should be in the following format: [https://elixir.bootlin.com/linux/v6.14/source/kernel/sched/core.c#L1234](https://elixir.bootlin.com/linux/v6.14/source/kernel/sched/core.c#L1234)
