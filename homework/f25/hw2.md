...
The system call you write will retrieve information from each thread associated
...
- **root_pid** represents the PID of the thread that will serve as the root of
...
- Retrieving some of the information for your `struct tskinfo` will be
  architecture-specific (i.e. it should be implemented in the `arch/arm64`
  directory rather than in generic kernel code). You should call generic
  functions from your main `kernel/ptree.c` file for retrieving these values,
  but implement them in `arch/arm64/kernel/ptree.c` (and make sure you modify
  the `Makefile` in the same folder appropriately). This ensures that only the
  correct retrieval function is included when your kernel is compiled. Note
  that you can put declarations for these functions in `include/linux/ptree.h`.
...
3. Give the exact URL on https://elixir.bootlin.com/linux/v7.0/source pointing
   ...
   answer for arm64. The function you identify, which may be in assembly code,
   should be the one that contains the actual instruction that switches the
   CPU's program counter register to the task so it can run. Note: use v7.0.
...
4. Give the exact URL on https://elixir.bootlin.com/linux/v7.0/source pointing
   ...
   the currently running process. Please provide an answer for arm64. Note: use
   v7.0.
...
where `x` represents the `userpc`, and `x` represents the `kernelpc`. Any valid
